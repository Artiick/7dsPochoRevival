...
class DogsFloor4BattleStrategy(IBattleStrategy):
    ...
    def _phase1_turn1_action(self, hand_of_cards: list[Card], card_turn: int):
        if card_turn == 0:
            idx = self._best_matching_card(hand_of_cards, ("thonar_stance",))
            if 0 <= idx < len(hand_of_cards):
                card = hand_of_cards[idx]
                if card.card_image is not None and find(vio.thonar_gauge, card.card_image):
                    return -1
            return idx
        if card_turn == 1:
        ...
        if card_turn == 3:
            return self._best_matching_card(hand_of_cards, ("nasi_heal",))
        return -1
    ...
    def _phase1_phase3_cleanup_action(
        ...
        protected_ids = set()
        protected_ids.update(self._phase1_keep_best_ids(hand_of_cards, ("nasi_stun",), keep_nasi_stun))
    ...
    def _phase1_keep_best_ids(
        self, hand_of_cards: list[Card], template_names: Sequence[str], keep_count: int
    ) -> set[int]:
        if keep_count <= 0:
            return set()
    ...
    def _phase2_stall_turn_action(self, hand_of_cards: list[Card]) -> int | list[int] | None:
        """One turn of moves / low cards when nasi_ult is not in hand or picked (playbook stall budget)."""
        if move_action := self._phase2_move_action(hand_of_cards):
            print("Phase 2 playbook: stalling with a merge/reposition move while nasi_ult is off-screen.")
            return move_action
        if last_resort_move := self._phase2_last_resort_move_action(hand_of_cards):
            print("Phase 2 playbook: stalling with a broader reposition move while nasi_ult is off-screen.")
            return last_resort_move
        for template_names in (("thonar_stance",), ("nasi_stun",), ("nasi_heal",)):
            card_idx = self._best_allowed_card(hand_of_cards, template_names)
            if card_idx != -1:
                print("Phase 2 playbook: stalling with a low-value card while nasi_ult is off-screen.")
                return card_idx
        return None
    ...
    def _phase2_turn1_action(self, hand_of_cards: list[Card], picked_cards: list[Card], card_turn: int):
        if card_turn == 0:
            ...
            opener = self._best_matching_card_with_ranks(
                hand_of_cards, ("nasi_stun",), {CardRanks.SILVER, CardRanks.GOLD}
            )
            if opener != -1:
                return opener
            ...
            opener = self._best_matching_card(hand_of_cards, ("nasi_stun",))
            if opener != -1:
                return opener
        ...
        for template_names in [("thonar_stance",), ("nasi_stun",)]:
            card_idx = self._best_allowed_card(hand_of_cards, template_names)
            if card_idx != -1:
                return card_idx
    ...
    def _phase2_turn2_action(self, hand_of_cards: list[Card], picked_cards: list[Card], card_turn: int):
        if card_turn == 0:
            nasi_heal = self._best_matching_card(hand_of_cards, ("nasi_heal",))
            if nasi_heal != -1:
        ...
        if card_turn == 0 and not self._has_card_in_hand_or_picked(hand_of_cards, picked_cards, ("nasi_ult",)):
            if DogsFloor4BattleStrategy.p2_stall_turns_used < 1:
                stall_action = self._phase2_stall_turn_action(hand_of_cards)
                if stall_action is not None:
                    DogsFloor4BattleStrategy._p2_stall_increment_pending = True
                    return stall_action
    ...
    def _phase2_fallback_action(self, hand_of_cards: list[Card], picked_cards: list[Card]):
        ...
        expendable_gauge = self._expendable_thonar_gauge_ids(hand_of_cards)
        if expendable_gauge:
            print("Phase 2 fallback: spending an expendable thonar_gauge to unblock the hand.")
            return self._lowest_expendable_thonar_gauge_id(hand_of_cards)
        visible_card = self._phase2_any_visible_card(hand_of_cards)
        if visible_card != -1:
            print(
                "Phase 2 fallback: no ideal safe action was found, but a visible filler card can still advance the run."
            )
            return visible_card
